...
from anagrams_2 import find_anagrams_2


class AnagramsTwoTest(unittest.TestCase):
    def setUp(self):
    ...
    def test_invalid_word_input(self):
        self.assertRaises(TypeError, find_anagrams_2, None, self.without_anagrams)

    def test_invalid_word_list_input(self):
        self.assertRaises(TypeError, find_anagrams_2, "test", None)

    def test_list_with_anagrams_only(self):
        initial_word = "trace"
        anagrams = find_anagrams_2(initial_word, self.with_anagrams_only)
        self.assertItemsEqual(anagrams, self.with_anagrams_only)

    def test_mixed(self):
        initial_word = "tread"
        anagrams = find_anagrams_2(initial_word, self.mixed)
        self.assertItemsEqual(anagrams, ["dater", "rated", "trade"])

    def test_wuthout_anagrams(self):
        initial_word = "sky"
        anagrams = find_anagrams_2(initial_word, self.without_anagrams)
        self.assertItemsEqual(anagrams, [])

    def test_inclusive(self):
        initial_word = "code"
        anagrams = find_anagrams_2(initial_word, self.inclusive)
